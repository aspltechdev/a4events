...
function Gallery() {
  ...
  const marqueeImages = images.length > 0 ? [...images, ...images, ...images] : [];

  return (
    <section
      ref={sectionRef}
      className={`gallery-pro-marquee ${isVisible ? "gallery-visible" : ""}`}
    >
      {/* Background */}
      <div className="gallery-bg-pro" aria-hidden="true">
        <div className="gallery-glow-pro gallery-glow-pro--teal" />
        <div className="gallery-glow-pro gallery-glow-pro--blue" />
        <div className="gallery-grain-pro" />
      </div>

      <div className="gallery-container-pro">
        {/* Header - Left aligned with right content */}
        <div className="gallery-header-pro">
          <div className="gallery-header-left">
            <div className="gallery-tag-pro">
              <span className="gallery-tag-dot" />
              <span>Visual Stories</span>
            </div>
            <h2 className="gallery-title-pro">
              <span className="gallery-title-line">Moments that</span>
              <span className="gallery-title-line gallery-title-highlight">create memories</span>
            </h2>
          </div>
          <div className="gallery-header-right">
            <p className="gallery-subtitle-pro">
              Explore highlights from concerts, festivals, and workshops captured through our lens
            </p>
            <div className="gallery-stats-pro">
              <span className="gallery-stat-item">
                <span className="gallery-stat-number">{images.length}+</span>
                <span className="gallery-stat-label">Moments</span>
              </span>
              <span className="gallery-stat-divider" />
              <span className="gallery-stat-item">
                <span className="gallery-stat-number">✦</span>
                <span className="gallery-stat-label">Curated</span>
              </span>
            </div>
          </div>
        </div>
        ...
        {/* Marquee Carousel */}
        <div
          className="gallery-marquee-pro"
          onMouseEnter={() => setIsPaused(true)}
          onMouseLeave={() => setIsPaused(false)}
        >
          <div className={`gallery-marquee-track-pro ${isPaused ? "gallery-paused" : ""}`}>
            {marqueeImages.map((item, index) => (
              <div
                key={`${item.id}-${index}`}
                className="gallery-marquee-item-pro"
                onClick={() => openLightbox(item)}
              >
                <div className="gallery-marquee-image-wrap-pro">
                  <img
                    src={`https://a4agroup.eu${item.image}`}
                    alt={item.title || "Gallery"}
                    className="gallery-marquee-image-pro"
                    loading="lazy"
                  />
                  <div className="gallery-marquee-overlay-pro">
                    <span className="gallery-marquee-icon-pro">+</span>
                    {item.title && (
                      <span className="gallery-marquee-title-pro">{item.title}</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          {/* Fade Edges */}
          <div className="gallery-marquee-fade-pro gallery-marquee-fade-pro-left" />
          <div className="gallery-marquee-fade-pro gallery-marquee-fade-pro-right" />
        </div>

        {/* Empty State */}
        {images.length === 0 && (
          <div className="gallery-empty-pro">
      ...
      {/* Lightbox */}
      {selectedImage && (
        <div className="gallery-lightbox-pro" onClick={closeLightbox}>
          <div className="gallery-lightbox-backdrop-pro" />
          <div className="gallery-lightbox-content-pro" onClick={(e) => e.stopPropagation()}>
            <button className="gallery-lightbox-close-pro" onClick={closeLightbox}>
              <span>✕</span>
            </button>

            <div className="gallery-lightbox-image-wrap-pro">
              <img
                src={`https://a4agroup.eu${selectedImage.image}`}
                alt={selectedImage.title || "Gallery"}
                className="gallery-lightbox-image-pro"
              />
            </div>

            {selectedImage.title && (
              <p className="gallery-lightbox-caption-pro">{selectedImage.title}</p>
            )}
          </div>
        </div>
      )}
