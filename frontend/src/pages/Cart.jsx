...
//       // Remove previously stored Sunday
//       if (isSunday(savedDate)) {
...
// Minimum allowed delivery date = today + 3 days
// (blocks today, tomorrow, and the day after tomorrow)
...
// Check if a date is within the blocked window (today, +1, +2)
const isTooSoon = (dateString) => {
  if (!dateString) return false;
...
const CartSummary = ({
  ...
  const [deliveryDateError, setDeliveryDateError] =
    useState("");
  ...
  const handleExpectedDeliveryDateChange = (e) => {
    ...
    if (!value) {
      ...
      try {
        ...
        console.warn(
          "Unable to clear expected delivery date:",
          error
        );
      }
    ...
    if (isSunday(value)) {
      setDeliveryDateError(
        "Sundays are not available for delivery. Please pick another date."
      );
      return;
    }

    // Do not allow the next 2 days (today, +1, +2)
    if (isTooSoon(value)) {
      setDeliveryDateError(
        "Delivery must be scheduled at least 3 days from today."
      );
      return;
    }
        ...
        <div className="cart-expected-delivery">
          ...
          {deliveryDateError && (
            <p
              ...
              className="expected-delivery-error"
              role="alert"
            >
              {deliveryDateError}
            </p>
          ...
          <p className="expected-delivery-note">
            Earliest available date is{" "}
            {getMinDeliveryDate()}. Sundays are not
            available.
          </p>
...
function Cart() {
  ...
  const [
    ...
    try {
      ...
      // Remove previously stored Sunday or too-soon date
      if (
        isSunday(savedDate) ||
        isTooSoon(savedDate)
      ) {
