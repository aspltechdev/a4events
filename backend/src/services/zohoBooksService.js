...
async function findCustomerByEmail(accessToken, email) {
  ...
  console.log("Searching Zoho customer by email:", email);
  ...
  try {
    ...
    if (contacts.length > 0) {
      console.log("Zoho customer found by email:", contacts[0].contact_id);
      return contacts[0];
    }

    console.log("No Zoho customer found by email");
    return null;
  } catch (error) {
    console.error("ZOHO FIND CUSTOMER BY EMAIL ERROR:");
    console.error(error.response?.data || error.message);
    return null;
  }
}
...
async function findCustomerByName(accessToken, customerName) {
  if (!customerName) return null;

  console.log("Searching Zoho customer by name:", customerName);
  ...
  try {
    ...
    if (contacts.length > 0) {
      console.log("Zoho customer found by name:", contacts[0].contact_id);
      return contacts[0];
    }

    console.log("No Zoho customer found by name");
    return null;
  } catch (error) {
    console.error("ZOHO FIND CUSTOMER BY NAME ERROR:");
    console.error(error.response?.data || error.message);
    return null;
  }
}
...
|--------------------------------------------------------------------------
|
| IMPORTANT: We intentionally DO NOT send tax_reg_no
| because your Zoho EU organization rejected it.
|
*/

async function createCustomer(accessToken, order) {
  ...
  console.log("Creating Zoho customer:", customerName);

  const customerData = {
    contact_name: customerName,
    contact_type: "customer",
    customer_sub_type: order.companyName ? "business" : "individual",
    email: order.email,
    phone: order.phone || undefined,
    company_name: order.companyName || undefined,
    billing_address: {
      attention: customerName,
    ...
    notes: `A4 Events Order: ${order.orderNumber}`,
  };

  // Remove undefined values and explicitly skip tax_reg_no
  function cleanObject(obj) {
    if (Array.isArray(obj)) {
  ...
  const cleanedCustomerData = cleanObject(customerData);
  ...
  try {
    ...
    if (!customer?.contact_id) {
      console.error("ZOHO CREATE CUSTOMER RESPONSE:", response.data);
      throw new Error("Zoho customer was not created");
    }

    console.log("Zoho customer created:", customer.contact_id);
    return customer;
  } catch (error) {
    console.error("ZOHO CREATE CUSTOMER ERROR:");
    console.error(error.response?.data || error.message);
    throw error;
  }
}
...
async function findOrCreateCustomer(accessToken, order) {
  // 1. Search by email
  const byEmail = await findCustomerByEmail(accessToken, order.email);
  if (byEmail) return byEmail;

  // 2. Search by name
  const customerName = `${order.firstName || ""} ${order.lastName || ""}`.trim();
  const byName = await findCustomerByName(accessToken, customerName);
  if (byName) return byName;

  // 3. Create customer
  return await createCustomer(accessToken, order);
}
...
async function findZohoItem(accessToken, itemName) {
  if (!itemName) return null;

  console.log("Searching Zoho item:", itemName);
  ...
  try {
    ...
    if (exact) {
      console.log("Zoho item found:", exact.item_id, exact.name, "type:", exact.item_type);
      return exact;
    }

    console.log("No exact Zoho item found:", itemName);
    return null;
  } catch (error) {
    console.error("ZOHO FIND ITEM ERROR:");
    console.error(error.response?.data || error.message);
    return null;
  }
}
...
async function createZohoSalesItem(accessToken, productName, rate, description) {
  const salesAccountId = process.env.ZOHO_SALES_ACCOUNT_ID;
  ...
  const cleanName = String(productName || "A4 Product").trim();
  const salesItemName = `A4 Sales - ${cleanName}`;

  console.log("Creating Zoho sales item:", salesItemName);
  ...
  const itemData = {
    ...
    unit: "unit",
    item_type: "sales",
    account_id: String(salesAccountId),
  };
  ...
  try {
    ...
    if (!item?.item_id) {
      console.error("ZOHO CREATE ITEM RESPONSE:", response.data);
      throw new Error("Zoho sales item was not created");
    }

    console.log("Zoho sales item created:", item.item_id);
    return item;
  } catch (error) {
    console.error("ZOHO CREATE SALES ITEM ERROR:");
    console.error(error.response?.data || error.message);
    throw error;
  }
}
...
async function getOrCreateSalesItem(accessToken, productName, rate, description) {
  const salesItemName = `A4 Sales - ${String(productName || "").trim()}`;

  const dedicatedSalesItem = await findZohoItem(accessToken, salesItemName);

  if (dedicatedSalesItem) {
    const itemType = String(
      dedicatedSalesItem.item_type || dedicatedSalesItem.product_type || ""
    ).toLowerCase();
    ...
    if (
      ...
      console.log("Using existing A4 sales item:", dedicatedSalesItem.item_id);
      return dedicatedSalesItem;
    }
  }

  return await createZohoSalesItem(accessToken, productName, rate, description);
}
...
async function buildInvoiceLineItems(accessToken, order) {
  ...
  for (const orderItem of items) {
    ...
    const rate = Number(orderItem.price || orderItem.unitPrice || 0);

    if (rate < 0) {
      throw new Error(`Invalid product price for ${productName}`);
    }

    const zohoItem = await getOrCreateSalesItem(
      accessToken,
      productName,
      rate,
      productName
    ...
    lineItems.push({
      item_id: String(zohoItem.item_id),
      name: productName,
...
async function createZohoInvoice(order) {
  console.log("========================================");
  ...
  try {
    ...
    const customer = await findOrCreateCustomer(accessToken, order);

    if (!customer?.contact_id) {
      throw new Error("Unable to obtain Zoho customer ID");
    }

    const lineItems = await buildInvoiceLineItems(accessToken, order);
    const totals = getOrderTotals(order);

    console.log("Zoho Customer ID:", customer.contact_id);
    console.log("Zoho Line Items:", JSON.stringify(lineItems, null, 2));
    ...
    const invoiceData = {
      ...
      shipping_charge: totals.deliveryCharge,
      notes: `A4 Events Order ${order.orderNumber}. Payment proof submitted. Payment verification is pending.`,
      terms: "Payment received by bank transfer is subject to verification by A4 Events.",
      send: false,
      allow_partial_payments: false,
    };

    console.log("Sending invoice request to Zoho...");
    ...
    if (!invoice?.invoice_id) {
      console.error("ZOHO CREATE INVOICE RESPONSE:", response.data);
      throw new Error("Zoho invoice was not created");
    }
    ...
    console.error("========================================");
    console.error("CREATE ZOHO INVOICE ERROR");
    console.error("Order:", order?.orderNumber);
    console.error("Message:", error.message);
    console.error("Zoho HTTP Status:", error.response?.status);
...
async function getZohoInvoice(invoiceId) {
  try {
    ...
    console.error("ZOHO GET INVOICE ERROR:");
    console.error(error.response?.data || error.message);
    throw error;
  }
}
...
async function markZohoInvoiceAsSent(invoiceId) {
  try {
    ...
    console.log("Zoho invoice marked as sent:", invoiceId);
    return response.data;
  } catch (error) {
    console.error("ZOHO MARK INVOICE SENT ERROR:");
    console.error(error.response?.data || error.message);
    throw error;
  }
}
